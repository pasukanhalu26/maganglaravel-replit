...
<div class="welcome-banner d-flex align-items-center justify-content-between flex-wrap gap-3">
    <div>
        ...
        <h3 class="fw-bold mb-1">Selamat datang, {{ str_replace('_', ' ', ucwords(Auth::user()->role)) }}! 👋</h3>
    ...
    <div class="d-flex align-items-center gap-2 flex-wrap">
        <div class="periode-chip">
            ...
            <select id="periodeFilter" class="form-select border-0 bg-transparent text-white fw-bold" style="min-width:80px;cursor:pointer" onchange="window.location='?periode='+this.value">
                ...
                <option value="{{ $yr }}" {{ $yr == $selectedYear ? 'selected' : '' }} style="color:#333">{{ $yr }}</option>
                @endforeach
            </select>
        </div>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-6 col-lg-3">
        <div class="kpi-card">
            <div class="kpi-card-body">
                ...
                <div class="kpi-value" style="color:#0f172a">{{ number_format($sum_capaian_kumulatif) }}</div>
                <div class="kpi-sub">dari target tahunan <strong style="color:#0f172a">{{ number_format($sum_target_tahunan) }}</strong></div>
                <div class="kpi-bar">
                    <div class="kpi-bar-fill" style="width:{{ min($overall_progress,100) }}%;background:#3b82f6"></div>
                </div>
                <div class="d-flex justify-content-between mt-1" style="font-size:.63rem;color:#94a3b8">
                    <span>0%</span>
                    <span class="fw-bold" style="color:#3b82f6">{{ $overall_progress }}% tercapai</span>
                    <span>100%</span>
                </div>
            </div>
        </div>
    ...
    {{-- KPI 2: Posisi vs Jadwal --}}
    <div class="col-6 col-lg-3">
        <div class="kpi-card" style="border-top: 3px solid {{ $on_schedule ? '#059669' : '#dc2626' }}">
            <div class="kpi-card-body">
                <div class="d-flex align-items-center gap-2 mb-3">
                    <div class="kpi-icon" style="background:{{ $on_schedule ? '#d1fae5' : '#fee2e2' }}">
                        <i class="fas fa-{{ $on_schedule ? 'check' : 'exclamation' }}" style="color:{{ $on_schedule ? '#059669' : '#dc2626' }}"></i>
                    </div>
                    <div class="kpi-label">Posisi vs Jadwal</div>
                </div>
                <div class="kpi-sub mb-1">Bln ke-{{ $current_month }}/12 — seharusnya sudah:</div>
                <div class="kpi-value" style="color:#0f172a;font-size:1.5rem">{{ number_format($expected_now) }}</div>
                @if($on_schedule)
                    <div class="mt-2 px-2 py-1 rounded-2 d-inline-flex align-items-center gap-1" style="background:#d1fae5;font-size:.7rem;color:#065f46;font-weight:700">
                        <i class="fas fa-arrow-up" style="font-size:.6rem"></i> Surplus {{ number_format($surplus) }} ✓
                    </div>
                @else
                    <div class="mt-2 px-2 py-1 rounded-2 d-inline-flex align-items-center gap-1" style="background:#fee2e2;font-size:.7rem;color:#991b1b;font-weight:700">
                        <i class="fas fa-arrow-down" style="font-size:.6rem"></i> Ketinggalan {{ number_format($deficit) }}
                    </div>
                @endif
                <div class="mt-2" style="font-size:.63rem;color:#94a3b8">
                    ({{ $current_month }}/12) × {{ number_format($sum_target_tahunan) }}
                </div>
            </div>
        </div>
    </div>

    {{-- KPI 3: Target Wajib/Bulan --}}
    <div class="col-6 col-lg-3">
        <div class="kpi-card" style="border-top: 3px solid {{ $need_extra ? '#d97706' : '#059669' }}">
            <div class="kpi-card-body">
                <div class="d-flex align-items-center gap-2 mb-3">
                    <div class="kpi-icon" style="background:{{ $need_extra ? '#fef3c7' : '#d1fae5' }}">
                        <i class="fas fa-calculator" style="color:{{ $need_extra ? '#d97706' : '#059669' }}"></i>
                    </div>
                    <div class="kpi-label">Target Wajib / Bulan</div>
                </div>
                <div class="kpi-sub mb-1">Agar target terpenuhi di Desember:</div>
                <div class="kpi-value" style="color:{{ $need_extra ? '#d97706' : '#059669' }}">{{ number_format($needed_per_month) }}</div>
                <div class="kpi-sub">/bulan × {{ $months_left }} bln sisa</div>
                @if($need_extra)
                    <div class="mt-2" style="font-size:.69rem;color:#d97706;font-weight:600">
                        <i class="fas fa-exclamation-triangle me-1"></i>Tren saat ini {{ number_format($avg_per_month) }}/bln — butuh +{{ number_format($needed_per_month - $avg_per_month) }}
                    </div>
                @else
                    <div class="mt-2" style="font-size:.69rem;color:#059669;font-weight:600">
                        <i class="fas fa-check-circle me-1"></i>Tren {{ number_format($avg_per_month) }}/bln — sudah aman ✓
                    </div>
                @endif
                <div class="mt-1" style="font-size:.63rem;color:#94a3b8">{{ number_format($remaining_target) }} ÷ {{ $months_left }} bln</div>
            </div>
        </div>
    </div>

    {{-- KPI 4: Status Indikator --}}
    <div class="col-6 col-lg-3">
        <div class="kpi-card">
            <div class="kpi-card-body">
                <div class="d-flex align-items-center gap-2 mb-3">
                    <div class="kpi-icon" style="background:#ede9fe"><i class="fas fa-list-check" style="color:#7c3aed"></i></div>
                    <div class="kpi-label">Status {{ $total_ind }} Indikator</div>
                </div>
                <div class="status-row-item" style="background:#d1fae5">
                    ...
                    <span style="font-size:1.1rem;font-weight:900;color:#059669">{{ $cnt_tercapai }}</span>
                </div>
                <div class="status-row-item" style="background:#dbeafe">
                    <span style="color:#1e40af"><i class="fas fa-equals me-1"></i>On Track</span>
                    <span style="font-size:1.1rem;font-weight:900;color:#3b82f6">{{ $cnt_ontrack }}</span>
                </div>
                <div class="status-row-item" style="background:#fee2e2">
                    ...
                    <span style="font-size:1.1rem;font-weight:900;color:#dc2626">{{ $cnt_behind + $cnt_belum }}</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="row g-4 mb-4">
    {{-- KOLOM KIRI: LEADERBOARD KINERJA DESA --}}
    <div class="col-lg-5">
                ...
                <div class="p-3">
                    ...
                    <div class="d-flex flex-column gap-2 mb-4">
                        ...
                        <div class="d-flex align-items-center justify-content-between p-2 rounded-3" style="background:#f0fdf4;border:1px solid #dcfce7">
                            <div class="d-flex align-items-center gap-2">
                                <div style="width:24px;height:24px;background:#d1fae5;color:#059669;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:0.75rem">{{ $loop->iteration }}</div>
                                <span class="fw-bold text-dark small">{{ $d['nama'] }}</span>
                            </div>
                            <span class="badge bg-success border-0">{{ $d['pct'] }}%</span>
                        </div>
                        @empty
                    ...
                    <div class="d-flex flex-column gap-2">
                        ...
                        <div class="d-flex align-items-center justify-content-between p-2 rounded-3" style="background:#fef2f2;border:1px solid #fee2e2">
                            ...
                            <span class="badge bg-danger border-0">{{ $d['pct'] }}%</span>
                        </div>
                        @empty
                        <div class="text-muted small">Data belum tersedia</div>
                        @endforelse
    ...
    {{-- KOLOM KANAN: PRIORITAS INTERVENSI --}}
    <div class="col-lg-7">
        <div class="kpi-card h-100" style="border-top:3px solid #dc2626">
            <div class="kpi-card-body p-0">
                ...
                <div class="p-3">
                    @forelse($indikator_darurat as $darurat)
                    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between p-3 rounded-3 mb-2" style="background:#fff;border:1px solid #e2e8f0;border-left:4px solid #dc2626">
                        <div class="mb-2 mb-md-0" style="max-width:350px">
                            <div class="fw-bold text-dark" style="font-size:0.8rem;line-height:1.3">{{ Str::limit($darurat['indikator'], 65) }}</div>
                            <div class="text-muted mt-1" style="font-size:0.65rem"><i class="fas fa-layer-group me-1"></i>{{ $darurat['klaster'] }}</div>
                        </div>
                        <div class="d-flex align-items-center gap-3">
                            <div class="text-end">
                                <div class="text-muted" style="font-size:0.65rem;text-transform:uppercase">Defisit Saat Ini</div>
                                <div class="fw-bold" style="color:#dc2626;font-size:0.95rem">-{{ number_format($darurat['gap']) }}</div>
                            </div>
                            ...
                            <div class="text-end" style="min-width:65px">
                                ...
                                <div class="fw-bold text-dark" style="font-size:0.95rem">{{ $darurat['proj_pct'] }}%</div>
                            </div>
                            <a href="{{ route('capaian.index', ['katakunci' => substr($darurat['indikator'], 0, 15)]) }}" class="btn btn-sm btn-light border" title="Input Capaian" style="border-radius:8px"><i class="fas fa-arrow-right text-primary"></i></a>
                        </div>
                    </div>
                    @empty
...
<div class="row g-3 mb-4">
@foreach($klaster_data as $kd)
<div class="col-md-4 col-xl-3">
<div class="klaster-card">
<div class="d-flex justify-content-between align-items-start mb-2">
<div>
<div class="fw-bold small">{{ $kd['nama'] }}</div>
<div class="text-muted" style="font-size:.72rem">{{ $kd['jumlah_indikator'] }} indikator</div>
</div>
<span class="badge {{ $kd['progress'] >= 100 ? 'badge-tercapai' : ($kd['progress'] >= ($current_month/12*100) ? 'badge-on-track' : 'badge-behind') }} badge-status">{{ $kd['progress'] }}%</span>
</div>
<div class="d-flex justify-content-between small text-muted mb-1">
<span>Capaian <strong class="text-dark">{{ number_format($kd['capaian']) }}</strong></span>
<span>Target <strong class="text-dark">{{ number_format($kd['target']) }}</strong></span>
</div>
<div class="progress-bar-custom">
<div class="progress-bar-fill" style="width:{{ min($kd['progress'],100) }}%;background:{{ $kd['progress'] >= 100 ? '#10b981' : ($kd['progress'] >= ($current_month/12*100) ? '#3498db' : '#e74c3c') }}"></div>
</div>
</div>
</div>
@endforeach
@if(empty($klaster_data))
<div class="col-12 text-center py-4 text-muted"><i class="fas fa-inbox fa-2x mb-2"></i><p class="mb-0">Belum ada data klaster</p></div>
@endif
</div>

<h6 class="fw-bold text-dark mb-3 mt-4"><i class="fas fa-map-marker-alt me-2 text-success"></i>Pencapaian per desa — {{ $selectedYear }}</h6>
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2 small text-muted">
<span>Target dibagi {{ $total_desa_count }} desa</span>
...
@foreach($desa_indikator_data as $idx => $desa)
@php
...
@endphp
<div class="mb-2">
<div class="d-flex align-items-center flex-wrap gap-2 p-3 rounded-3" style="background:{{ $headerColor }};border:1px solid {{ $headerBorder }};cursor:pointer" data-bs-toggle="collapse" data-bs-target="#desaCollapse{{ $idx }}">
    <div class="flex-grow-1 d-flex align-items-center gap-2 flex-wrap" style="min-width:180px">
        <div class="fw-bold small" style="min-width:110px">{{ $desa['nama'] }}</div>
        <div class="d-flex align-items-center gap-2 flex-grow-1" style="min-width:140px">
            <div style="height:8px;flex:1;background:rgba(0,0,0,.06);border-radius:4px;overflow:hidden;max-width:220px">
                <div style="height:100%;width:{{ $pctTerpenuhi }}%;background:{{ $desa['terpenuhi'] == $desa['total'] && $desa['total'] > 0 ? '#10b981' : '#3498db' }};border-radius:4px"></div>
            </div>
            <span class="fw-bold small" style="color:{{ $desa['terpenuhi'] == $desa['total'] && $desa['total'] > 0 ? '#059669' : '#3498db' }}">{{ $pctTerpenuhi }}%</span>
        </div>
    </div>
    <div class="d-flex align-items-center gap-3 ms-auto flex-wrap">
        <div class="text-center"><div class="text-uppercase" style="font-size:.62rem;color:#666">Terpenuhi</div><div class="fw-bold" style="color:#059669;font-size:.95rem">{{ $desa['terpenuhi'] }}</div></div>
        <div class="text-center"><div class="text-uppercase" style="font-size:.62rem;color:#666">Belum</div><div class="fw-bold" style="color:#e74c3c;font-size:.95rem">{{ $desa['belum'] }}</div></div>
        <div class="text-center"><div class="text-uppercase" style="font-size:.62rem;color:#666">Total</div><div class="fw-bold text-dark" style="font-size:.95rem">{{ $desa['total'] }}</div></div>
        <i class="fas fa-chevron-down text-muted" style="font-size:.75rem"></i>
    </div>
</div>
<div class="collapse" id="desaCollapse{{ $idx }}">
<div class="p-0 mt-1">
...
@foreach($bulan_names as $bn)
<th class="text-center py-2 text-uppercase" style="font-size:.68rem;font-weight:700;color:#6b7280;">{{ $bn }}</th>
@endforeach
...
<tbody>
@foreach($desa['indikators'] as $ind)
<tr style="{{ $ind['tercapai'] ? 'background:rgba(16,185,129,.04)' : '' }}">
<td class="ps-3" style="max-width:220px;position:sticky;left:0;background:{{ $ind['tercapai'] ? '#f4fdf8' : '#fff' }};z-index:1;box-shadow: 2px 0 5px rgba(0,0,0,0.02);">
    <div class="fw-bold" style="font-size:0.75rem;line-height:1.2;">{{ Str::limit($ind['indikator'], 60) }}</div>
    <div class="text-muted mt-1" style="font-size:0.65rem"><i class="fas fa-tag me-1"></i>{{ Str::limit($ind['klaster'], 30) }}</div>
</td>
<td class="text-center align-middle fw-bold" style="font-size:0.8rem;">{{ number_format($ind['target_desa']) }}</td>
<td class="text-center align-middle text-muted" style="font-size:0.7rem;">±{{ number_format($ind['target_bulan']) }}</td>
@for($m = 1; $m <= 12; $m++)
    @php 
    ...
    @endphp
    <td class="text-center align-middle" style="background:{{ $bg_color }}; color:{{ $text_color }}; font-weight:{{ $cap_b > 0 ? '700' : '400' }}; font-size:0.7rem;">
        {{ $cap_b > 0 ? number_format($cap_b) : '-' }}
    </td>
@endfor
<td class="text-center align-middle fw-bold" style="color:#0f172a;font-size:0.8rem;">{{ number_format($ind['capaian']) }}</td>
<td class="text-center align-middle fw-bold" style="font-size:0.75rem; color:{{ $ind['sisa'] <= 0 ? '#059669' : '#e74c3c' }}">
    @if($ind['sisa'] <= 0)
        {{ $ind['sisa'] == 0 ? '0 ✓' : 'Over +'.number_format(abs($ind['sisa'])) }}
    @else
        {{ number_format($ind['sisa']) }}
    @endif
</td>
<td class="text-center align-middle">
    ...
    @else
        <span class="badge-status badge-behind" style="font-size:0.65rem">Kurang {{ number_format($ind['sisa']) }}</span>
    @endif
</td>
</tr>
@endforeach
...
<div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
    <div>
        <h6 class="fw-bold mb-1 text-dark" style="font-size:1rem">
            ...
            Proyeksi Cerdas Akhir Tahun {{ $selectedYear }}
        </h6>
        <div class="small text-muted">Berdasarkan rata-rata kecepatan capaian {{ $current_month }} bulan terakhir &mdash; dihitung otomatis tiap hari.</div>
    </div>
    <div class="d-flex gap-2 flex-wrap">
        <span class="px-3 py-1 rounded-pill fw-bold small" style="background:#fee2e2;color:#b91c1c"><i class="fas fa-times-circle me-1"></i>{{ $kritis_count }} Kritis</span>
        <span class="px-3 py-1 rounded-pill fw-bold small" style="background:#fef3c7;color:#92400e"><i class="fas fa-exclamation-triangle me-1"></i>{{ $waspada_count }} Waspada</span>
        <span class="px-3 py-1 rounded-pill fw-bold small" style="background:#d1fae5;color:#065f46"><i class="fas fa-check-circle me-1"></i>{{ $aman_count }} Aman</span>
    </div>
</div>
...
<div class="p-4 rounded-3 mb-4" style="background: linear-gradient(135deg, #7c3aed, #4f46e5); color: #fff; box-shadow: 0 8px 24px rgba(124,58,237,0.2);">
    <div class="row g-4 align-items-center">
        <div class="col-md-5">
            <div class="mb-2" style="font-size:0.75rem;opacity:0.8;text-transform:uppercase;letter-spacing:.06em">Proyeksi Capaian Global Akhir {{ $selectedYear }}</div>
            <div style="font-size:3rem;font-weight:900;line-height:1">{{ $projected_pct }}%</div>
            <div class="mt-1" style="font-size:0.8rem;opacity:0.85">
                Dengan tren saat ini, estimasi capaian tahunan: <strong>{{ number_format($projected_total) }}</strong> dari <strong>{{ number_format($sum_target_tahunan) }}</strong>
            </div>
            <div class="mt-2">
                @if($overall_verdict === 'aman')
                ...
                @elseif($overall_verdict === 'waspada')
            ...
            <div class="row g-3">
                <div class="col-6">
                    <div class="p-3 rounded-2" style="background:rgba(255,255,255,0.12)">
                        ...
                        <div class="fw-bold mt-1" style="font-size:1.4rem">{{ number_format($avg_per_month) }}</div>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-3 rounded-2" style="background:rgba(255,255,255,0.12)">
                        <div style="font-size:0.7rem;opacity:0.75;text-transform:uppercase;letter-spacing:.05em">Yang Harus Dicapai/Bulan</div>
                        <div class="fw-bold mt-1" style="font-size:1.4rem;color:{{ $needed_per_month > $avg_per_month ? '#fca5a5' : '#6ee7b7' }}">{{ number_format($needed_per_month) }}</div>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-3 rounded-2" style="background:rgba(255,255,255,0.12)">
                        <div style="font-size:0.7rem;opacity:0.75;text-transform:uppercase;letter-spacing:.05em">Sisa Bulan</div>
                        <div class="fw-bold mt-1" style="font-size:1.4rem">{{ $months_left }} bln</div>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-3 rounded-2" style="background:rgba(255,255,255,0.12)">
                        <div style="font-size:0.7rem;opacity:0.75;text-transform:uppercase;letter-spacing:.05em">Total Defisit Saat Ini</div>
                        <div class="fw-bold mt-1" style="font-size:1.4rem;color:#fca5a5">{{ number_format(max(0, $gap_to_target)) }}</div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="mb-3 d-flex align-items-center gap-2 flex-wrap">
    ...
    <span class="ms-auto small text-muted"><i class="fas fa-calendar me-1"></i>Bulan sekarang: <strong>{{ $bulan_indo[$current_month] }}</strong></span>
</div>
...
@endphp
<tr style="{{ $fi['verdict'] === 'kritis' ? 'background: rgba(220,38,38,0.02)' : '' }}">
    <td class="ps-3 align-middle" style="max-width:220px">
        <div class="fw-bold text-dark" style="font-size:0.78rem;line-height:1.3">{{ Str::limit($fi['indikator'], 60) }}</div>
        <div class="text-muted mt-1" style="font-size:0.65rem"><i class="fas fa-layer-group me-1"></i>{{ Str::limit($fi['klaster'], 28) }}</div>
    </td>
    <td class="text-center align-middle fw-bold">{{ number_format($fi['target']) }}</td>
    <td class="text-center align-middle">
        <div class="fw-bold" style="color:#3498db">{{ number_format($fi['capaian']) }}</div>
        <div style="font-size:0.65rem;color:#94a3b8">{{ $fi['capaian'] > 0 ? round(($fi['capaian']/$fi['target'])*100, 1).'%' : '0%' }}</div>
    </td>
    <td class="text-center align-middle text-muted">{{ number_format($fi['avg_pm']) }}</td>
    <td class="text-center align-middle">
        <span class="fw-bold" style="color: {{ $need_extra ? '#dc2626' : '#059669' }}">{{ number_format($fi['needed_pm']) }}</span>
        @if($need_extra && $fi['avg_pm'] > 0)
        <div style="font-size:0.62rem;color:#dc2626">+{{ round((($fi['ratio']-1)*100)) }}% dari tren</div>
        @endif
    </td>
    <td class="text-center align-middle">
        <div class="fw-bold" style="color: {{ $verdict_color }}">{{ $fi['proj_pct'] }}%</div>
        <div class="mx-auto mt-1" style="width:80px;height:4px;background:#f1f5f9;border-radius:4px;overflow:hidden">
            <div style="height:100%;width:{{ min($fi['proj_pct'], 100) }}%;background:{{ $verdict_color }};border-radius:4px"></div>
        </div>
    </td>
    <td class="text-center align-middle pe-3">
        <span class="px-2 py-1 rounded-pill fw-bold" style="background:{{ $verdict_bg }};color:{{ $verdict_color }};font-size:0.68rem;white-space:nowrap">{{ $verdict_text }}</span>
    </td>
</tr>
@empty
