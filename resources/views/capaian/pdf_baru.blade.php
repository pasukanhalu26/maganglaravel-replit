...
<body>
    ...
    <div class="header-title">
        PENCAPAIAN BULANAN KLASTER 2 UKM PUSKESMAS DRIYOREJO TAHUN {{ $tahunTampil }}
    </div>

    @foreach($data->groupBy('bulan') as $bulanVal => $itemsByBulan)
        @php
            $namaBulan = $bulansArr[$bulanVal] ?? 'BULAN ' . $bulanVal;
        @endphp

        @foreach($itemsByBulan->groupBy(function($item) { return $item->target->program->nama_program ?? 'Program Lainnya'; }) as $namaProgram => $items)
            @php
                ...
                <tbody>
                    @foreach($items as $index => $item)
                        ...
                        <tr>
                            ...
                            <td class="text-center bg-green">{{ number_format($capaian_bulanan) }}</td>
                            <td class="text-center">{{ number_format($kumulatif) }}</td>
                            <td class="text-center">{{ number_format($persenKumulatif, 0) }}%</td>
                            
                            @if($index === 0)
                                <td class="text-center" rowspan="{{ count($items) }}" style="vertical-align: middle;">
                                    {{ number_format($persenProgram, 2) }}%
                                </td>
                            @endif
                            
                            <td class="bg-green">{{ $analisaMasalah }}</td>
                            <td class="bg-green">{{ $rtl }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
